...
class TelegramService:
    ...
    def _log_alert_decision(
        ...
        logger.debug(
            ...
            "threshold=%.2f threshold_fallback_used=%s current=%.2f peak=%.2f avg_real=%s window=%s "
    ...
    def _dispatch_alerts(
        ...
        *,
        ...
        for user in users:
            ...
            self._register_hysteresis_release(user, threshold, peak_value, now, last_alert_sent_at)
            ...
            if peak_value < threshold:
                ...
                self._record_premium_sample(
                    ...
                    self._next_allowed_at_renotify(last_alert_sent_at),
                    user.has_premium_access,
                    False,
                    "below_threshold",
                )
    ...
    def _register_hysteresis_release(
        ...
        reset_exists = (
            Event.query.filter(
                ...
                Event.timestamp > last_alert_sent_at,
            )
            .order_by(Event.timestamp.desc())
            .first()
        )
    ...
    def _process_premium_user(
        ...
        renotify_due = (
            last_alert_sent_at is not None
            and now - last_alert_sent_at >= self.RENOTIFY_INTERVAL
        )
    ...
    def _passed_hysteresis(
        ...
        return self._has_hysteresis_reset_since(user, last_alert_sent_at)
    ...
    def _next_allowed_at_renotify(
        self, last_alert_sent_at: Optional[datetime]
    ) -> Optional[datetime]:
        if not last_alert_sent_at:
            return None
        return last_alert_sent_at + self.RENOTIFY_INTERVAL
    ...
    def simulate_premium_alert_flow(
        ...
        *,
        ...
        for index, value in enumerate(values):
            ...
            elif last_alert_sent_at and now - last_alert_sent_at < rate_limit:
                reason = "cooldown"
            else:
                rearmed = (
                    last_alert_sent_at is None
                    or value <= lower_bound
                    or (
                        last_hysteresis_reset_at
                        and last_hysteresis_reset_at > last_alert_sent_at
                    )
                )
                renotify_due = (
                    last_alert_sent_at is not None
                    and now - last_alert_sent_at >= renotify_interval
                )
            ...
            results.append(
                {
                    ...
                    "sent": sent,
                    "reason": reason,
                    "last_alert_sent_at": last_alert_sent_at.isoformat()
